### This script has the code for running the western gray foxes only.
### For running East and West together see HZAR_AllData.R
### For running East on its own see HZAR_East.R
...
## Read in Data
Nuclear <- read.csv("GrayFox_HZAR_WestOnly.csv", header=T, stringsAsFactors=F) 

### first create vector with distance based intervals
intervals <- seq(from = -2120, to = 2120, by =20)
...
d <- read.csv("GrayFox_distances_ForHZAR_WestOnly_0.5splinemodel.csv", header=T, stringsAsFactors=F)
...
##Can plot them to obseverve differences
...
pdf("GrayFox_hzarcline_West_EBK0.5_tails.none.pdf",6,6)
...
### first create vector with distance based intervals
intervals <- seq(from = -2180, to = 2180, by =20)
...
#Read in distances of "sampling locations"
d <- read.csv("GrayFox_distances_ForHZAR_WestOnly_AHMMmodel.csv", header=T, stringsAsFactors=F)
...
#PDF graph version of top model
pdf("GrayFox_hzarcline_West_AHMMshiftedcenter_tails.none.pdf",6,6)
hzar.plot.fzCline(fit3_none,xlab="Distance (km) from Cline Center",ylab="Proportion of Eastern Gray Fox Ancestry",main="GF.hzar.plot.cline(tails = none)")
#hzar.plot.cline(myfitlist_mtDNA[[3]],xlab="Smoothed coastline distance in km (AK to CA)",ylab="American mtDNA",main="hzar.plot.cline()")
dev.off()
